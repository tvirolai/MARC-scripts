...
Huom! Oletuksena on, että palvelimella on 10 tiedostoa (alina00.seq - alina09.seq). Jos tiedosto on olemassa, se poistetaan. */
...
function download(urls, outputStream) {
  ...
  http.get(urls.shift(), (response) => { response.pipe(outputStream);
    outputStream.on('finish', () => { 
      if (urls.length) {
        download(urls, fs.createWriteStream('../data.seq', { flags: 'a' }));
...
function getUrls() {
  return _.map(_.range(0,10), (d) => 'http://replikointi-kk.lib.helsinki.fi/index/alina0' + d + '.seq');
}